...
public class AccountsFile implements AccountsFileInterface {
    ...
    public void loadFile() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("users.txt"))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                //treat pipe as a normal character instead of as 'or' in regex
                String[] info = line.split("\\|");
                
                //safety check to skip empty lines
                ...
                //use regex to remove 'IESEBA35330' first and leave the digits to be incremented
                nextAccountNum=Math.max(nextAccountNum, Long.parseLong(accountNum.replaceFirst("^IESEBA35330", ""))+1);
    ...
    //increment nextid for use in Authentication to form accountId when a new account is created and saved
    @Override
    ...
    //increment nextAccountNum for use in Authentication to form accountNum when a new account is created and saved
    @Override
    ...
    //increment nextRegistrationNum for use in Authentication to form registrationNum when a new account is created and saved
    @Override
    ...
    //method to reload AccountsFile after update and registering so you don't need to close and reopen app
    @Override
    public void reload(){
         loadFile();
    }
